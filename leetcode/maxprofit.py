...
class Solution(object):
    def maxProfit(self, prices):
        ...
        """
        # days = len(prices)
        # profit = [[0] * days for i in xrange(days)]
        # # 跨越的天数
        # for i in xrange(1, days):
        #     # 横轴坐标
        #     for j in xrange(days-i):
        #         k = j + i
        #         profit[i][k] = prices[k] - prices[i]
        #         for m in xrange(i,k+1):
        #             profit[i][k] = max(profit[i][k], profit[i][m-1] + profit[m+1][k])
        days = len(prices)
        profit = [[0] * (days+2) for i in xrange(days+2)]
        # 跨越的天数,从1天到days-1天
        for i in xrange(1, days):
            # 横轴坐标,从坐标1开始
            for j in xrange(1, days+1-i):
                k = j + i
                profit[j][k] = prices[k-1] - prices[j-1]
                for m in xrange(j,k+1):
                    profit[j][k] = max(profit[j][k], profit[j][m-1] + profit[m+1][k])
        # print profit
        # maxprofit = 0
        # for i in xrange(1, days+1):
        #     for j in xrange(1, days+1):
        #         maxprofit = maxprofit if maxprofit > profit[i][j] else profit[i][j]
        return profit[1][days]
...
#         int[][] profit = new int[days+2][days+2];
#         int i, j, k;
#         for(i = 1; i <= days-1; i++ ) {
#             for(j = 1; j<=days-i; j++) {
#                 k = j + i;
#                 profit[j][k] = prices[k-1] - prices[j-1];
#                 for(int m = j; m <=k; m++) {
#                     profit[j][k] = Math.max(profit[j][k], profit[j][m-1] + profit[m+1][k]);
#                 }
